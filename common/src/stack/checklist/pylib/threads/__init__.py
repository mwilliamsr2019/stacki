...
class LogParser(threading.Thread):
	...
	SSL_ACCESS_LOG_MAP = {
		'/install/sbin/profile.cgi?' : State.Autoyast_Sent,
	...
	def __init__(self, path, queue):
		...
		elif '/var/log/apache2/access_log' in path:
...
class BackendExec(threading.Thread):
	...
	def __init__(self, ip, queue, ignoreError=True):
		super(self.__class__, self).__init__()
		self.ip = ip
		self.queue = queue
	...
	def run(self):
		...
		sftp.put('/opt/stack/share/BackendTest.py', '/tmp/BackendTest.py')
		sftp.close()
...
class InstallHashProcessor(stack.mq.processors.ProcessorBase):
	"""
	Listens for Post Install Hash messages from the backend and adds
	it to the Queue.
	"""
	def isActive(self):
		return True
	...
	def __init__(self, context, sock, queue):
		stack.mq.processors.ProcessorBase.__init__(self, context, sock)
		self.queue = queue

		parentLog = logging.getLogger("checklist")
		self.log = parentLog.getChild("installhashprocessor")
	...
	def process(self, message):
		...
		for l in payload:
			msg_list.append('InstallHash' + ' - ' + l['name'] \
				+ ' : ' + l['hash'])

		sm = StateMessage(source, State.Reboot_Okay, False, \
			time.time(), '; '.join(msg_list))
		self.queue.put(sm)
...
class MQProcessor(stack.mq.processors.ProcessorBase):
	...
	def process(self, message):
		...
		try:
			...
			self.log.warn('Error %s parsing string - %s' % (str(e), payload))
