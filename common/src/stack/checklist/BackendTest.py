...
def outputState(state, flag, msg=''):
	os.environ["LD_LIBRARY_PATH"] = "/opt/stack/lib/"
...
class YastLogReader(threading.Thread):
	...
	def run(self):
		...
		q = deque( maxlen=3 )
		with open(YastLogReader.YAST_LOG, "r") as file:
			file.seek(0, 2)
			...
			while True:
				...
				if not line:
					...
					if q and 'YPushButton.cc' in q[-1]:
						...
						#
						# If log has not been updated in the past 5 minutes,
						# the installation maybe stalled
						#
						if int(currentTime - modifiedTime) > 60:
							msg = []
							for elem in q:
