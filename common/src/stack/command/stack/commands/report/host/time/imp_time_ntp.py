...
class Implementation(stack.commands.Implementation):	
	...
	def run(self, host):
		...
		if self.owner.osversion == '11.x':
			...
			service_name = "ntpd"
		self.owner.addOutput(host, f'/sbin/chkconfig {service_name} on')
		...
		#
		self.owner.addOutput(host, f'service {service_name} stop')
		self.owner.addOutput(host, '/usr/sbin/ntpdate %s' % self.owner.timeservers[0])
		self.owner.addOutput(host, f'service {service_name} start')
